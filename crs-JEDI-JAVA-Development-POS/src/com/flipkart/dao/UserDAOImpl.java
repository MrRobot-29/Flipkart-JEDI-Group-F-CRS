		...
		if (role.equals("professor")) {
			...
			try {
				...
				while (rs.next()) {
					...
					PreparedStatement stmt1 = conn
							.prepareStatement("SELECT * FROM Professor WHERE email = '" + userName + "'");
					ResultSet rs1 = stmt1.executeQuery();
					while (rs1.next()) {
						int profId = rs1.getInt("prof_id");
						String dept = rs1.getString("department");
						p = new Professor(userName, name, Role.PROFESSOR, Password, Gender.MALE, "India", "India", dept,
								"Asc. Prof", profId);
					}
				}
			...
			try {
				...
				while (rs.next()) {
					String name = rs.getString("name");
					PreparedStatement stmt1 = conn
							.prepareStatement("SELECT * FROM Student WHERE email = '" + userName + "'");
					ResultSet rs1 = stmt1.executeQuery();
					while (rs1.next()) {
						...
						std = new Student(userName, name, Role.STUDENT, Password, Gender.MALE, "India", "India", branch,
								stdId, sem, approvalStatus == 1);
					}
				}
